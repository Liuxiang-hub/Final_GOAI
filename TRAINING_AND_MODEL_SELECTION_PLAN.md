...
> 状态（2026-08-31）：2 × RTX 6000D Expert-only 训练与离线筛选已经完成。`global_step_7774`（1.75 epoch）为当前唯一离线部署候选；它仍需通过双 PIPER 低速闭环真机验证，不能把离线误差直接表述为任务成功率。
...
| 7,774 | 1.75 epoch：进入完整 episode 复评 Top-3，最终排名第 1 |
| 8,884 | 2.00 epoch：训练终点并进入完整 episode 复评 Top-3 |

正式训练最终保存 step 2,221 / 3,332 / 4,442 / 5,553 / 6,663 / 7,774 / 8,884 七个可续训检查点。完整评估显示 1.75 epoch 综合最优，因此当前不继续增加 epoch；如真机验证暴露欠拟合，再从完整训练状态有依据地续训。
...
离线阶段已选出 **Top-3**：step 6,663 / 7,774 / 8,884。完整 episode 同配置复评的最终排序为 **7,774 > 8,884 > 6,663**；测试结果不再用于继续调参。
...
2. 当前唯一离线部署候选为 **`global_step_7774`（1.75 epoch）**，2.00 epoch 仅保留为可追溯备选。
