...
> 状态（2026-09-17）：2 × RTX 6000D Expert-only训练、60条完整验证选模和30条完整冻结测试已经完成。`global_step_8884`（2.00 epoch）为当前唯一真机候选；RTC异步部署代码已完成CPU验证，尚待GPU延迟与双PIPER低速闭环验证。历史离线指标来自RTC之前的四块时序回退基线，不能当作RTC或真机成功率。
...
| RTX 6000D 数量 | 规划单步时间 | 8,884 steps 纯训练 | 加 10% 运行开销 | 一天内完成判断 |
|---:|---:|---:|---:|---|
| 1 | 14–18 s | 34.5–44.4 h | 38.0–48.9 h | 不可行 |
| 2 | 8–11.5 s | 19.7–28.4 h | 21.7–31.2 h | 临界，不保证 |
| **4** | **4.5–7.2 s** | **11.1–17.8 h** | **12.2–19.5 h** | **推荐** |
| 8 | 2.8–4.2 s | 6.9–10.4 h | 7.6–11.4 h | 可行，但性价比较低 |
...
历史上曾使用 test split 前500帧初筛七个候选；流程纠正后，只用固定的60条 validation episodes 对 1.50 / 1.75 / 2.00 epoch 三个候选统一运行六任务完整 episode。本轮实际选模指标为：

- 动作 MSE、MAE 与最差任务 MSE；
- Velocity RMSE 与静止段速度 RMS；
- Jerk RMS 与 Jerk P99.9 尾部尖峰。
...
### 第三级：安全回放与双 PIPER 验证（待完成）

离线阶段已经选出唯一真机候选 `global_step_8884`。该模型先经过：

1. 动作反归一化检查；
2. 关节顺序、单位、方向、夹爪范围检查；
3. 速度、加速度、jerk 和关节限位检查；
4. 低速空载回放；
5. 在人工急停监护下进入双 PIPER，每任务先执行 3 次低速初筛试验。

记录成功率、首次成功时间、人工急停次数、碰撞/越界次数和任务间最低成功率；不再用离线落选模型消耗真机试验预算。

### 第四级：比赛真机确认（待完成）

`global_step_8884` 在双 PIPER 上执行：

- 六任务各至少 5 次，共至少 30 次正式试验；
- 初始物体位置做比赛允许范围内扰动；
- 相机曝光、光照和背景做小范围变化；
- 统计总成功率与最差任务成功率；
...
3. 当前首选部署入口为RTC异步推理：H=50、25Hz、最短15步启动后台推理、PiGDM软约束、按实际延迟对齐；旧4-chunk时序集成/自适应EMA/振荡抑制保留为回退，不与RTC默认叠加。
