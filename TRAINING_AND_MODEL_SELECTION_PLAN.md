...
> 状态（2026-09-17）：2 × RTX 6000D Expert-only训练、60条完整验证选模和30条完整冻结测试已经完成。`global_step_8884`（2.00 epoch）为当前唯一真机候选；RTC异步部署代码已完成CPU验证，尚待GPU延迟与双PIPER低速闭环验证。历史离线图来自RTC之前的执行端，不能当作RTC或真机成功率。
...
3. 当前唯一部署入口为RTC异步推理：H=50、25Hz、最短15步启动后台推理、PiGDM软约束、按实际延迟对齐；旧4-chunk时序集成、自适应EMA和振荡抑制已移除，异常时安全停止。
