...
当前首选方案冻结 LingBot-VLA 2.0 `global_step_8884`，输出50-step、14维双
PIPER动作块；RTC在旧块执行期间异步生成新块，使用PiGDM软约束并按真实延迟
对齐切换。旧四块时序集成、自适应EMA和振荡抑制仅作为回退链路。

Q升级在RTC引导采样与机器人安全层之间增加候选决策。它不会替换
...
| 路径 | 状态 | 用途 |
|---|---|---|
| Baseline：step8884 + RTC | 当前首选、待GPU/真机验证 | 消除推理停顿并保持跨块连续 |
| Rollback：step8884 + 旧时序后处理 | 可用回退 | RTC异常时恢复离线验证链路 |
| Upgrade：step8884 + RTC + Q-Planning | 默认关闭 | 收集真机结果后进行A/B验证 |
...
- N=4时规划p95小于500ms，并且p99小于RTC旧块剩余35步形成的1.4秒硬缓冲；
- Q 异常、超时或模型缺失时无状态切回 baseline 第一候选；
- Q选择后进入RTC时间对齐与机器人安全限制；旧时序集成不与RTC默认叠加。
...
1. RTC baseline先完成GPU延迟、静止干跑和六任务低速真机测试，并能在至少部分任务产生成功轨迹；
